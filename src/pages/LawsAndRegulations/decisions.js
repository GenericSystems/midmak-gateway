import React, { Component } from "react";
import {
  Row,
  Col,
  Card,
  Alert,
  CardBody,
  Modal,
  ModalHeader,
  ModalBody,
  NavItem,
  NavLink,
  TabContent,
  TabPane,
  Nav,
  Input,
} from "reactstrap";
import BootstrapTable from "react-bootstrap-table-next";
import cellEditFactory from "react-bootstrap-table2-editor";
import AddIcon from "@mui/icons-material/Add";
import Tooltip from "@mui/material/Tooltip";
import DeleteIcon from "@mui/icons-material/Delete";
import { IconButton } from "@mui/material";
import Breadcrumbs from "../../components/Common/Breadcrumb";
import { withTranslation } from "react-i18next";
import { connect } from "react-redux";
import ToolkitProvider, {
  Search,
} from "react-bootstrap-table2-toolkit/dist/react-bootstrap-table2-toolkit";
import Select from "react-select";
import {
  getDecisions,
  addNewDecision,
  updateDecision,
  deleteDecision,
  getDecisionDeletedValue,
  getDecisionsRulesReasons,
  addNewDecisionsRulesReason,
  updateDecisionsRulesReason,
  deleteDecisionsRulesReason,
  getDecisionsRulesReasonDeletedValue,
  getDecisionsRulesCanceledReasons,
  addNewDecisionsRulesCanceledReason,
  updateDecisionsRulesCanceledReason,
  deleteDecisionsRulesCanceledReason,
  getDecisionsRulesCanceledReasonDeletedValue,
  getDecisionsRulesRoles,
  addNewDecisionsRulesRole,
  updateDecisionsRulesRole,
  deleteDecisionsRulesRole,
  getDecisionsRulesRoleDeletedValue,
} from "store/decisions/actions";
import { getRoles } from "store/roles/actions";
import Accordion from "react-bootstrap/Accordion";

import paginationFactory, {
  PaginationProvider,
  PaginationListStandalone,
} from "react-bootstrap-table2-paginator";
import { withRouter, Link } from "react-router-dom";
import DeleteModal from "components/Common/DeleteModal";
import classnames from "classnames";
import {
  checkIsAddForPage,
  checkIsDeleteForPage,
  checkIsEditForPage,
  checkIsSearchForPage,
} from "../../utils/menuUtils";
class DecisionsList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      deleteModal: false,
      duplicateError: null,
      selectedRowId: null,
      showAlert: null,
      isModalOpen: false,
      activeTab: "1",
      reasonArray: null,
      duplicateDecisionError: null,
      selectedMulti: [],
      decisionObj: "",
      showAddButton: false,
      showDeleteButton: false,
      showEditButton: false,
      showSearchButton: false,
    };
    this.toggleModal = this.toggleModal.bind(this);
    this.toggleTab = this.toggleTab.bind(this);
  }
  toggleTab(tab) {
    const { onGetDecisionsRulesRoles } = this.props;
    const { reasonArray } = this.state;
    if (this.state.activeTab !== tab) {
      this.setState({
        activeTab: tab,
        duplicateDecisionError: null,
      });
    }
    if (tab === "3") {
      onGetDecisionsRulesRoles({
        Id: reasonArray.Id,
        tablename: "settings_DecisionsRulesAcceptRoles",
      });
    } else if (tab === "4") {
      onGetDecisionsRulesRoles({
        Id: reasonArray.Id,
        tablename: "settings_DecisionsRulesRefuseRoles",
      });
    }
  }
  toggleModal() {
    const { onGetDecisions } = this.props;
    this.setState(prevState => ({
      isModalOpen: !prevState.isModalOpen,
      duplicateError: null,
    }));
    onGetDecisions();
  }

  onClickReason = reason => {
    const { onGetDecisionsRulesReasons, onGetDecisionsRulesCanceledReasons } =
      this.props;
    this.setState({
      decisionObj: reason,
      activeTab: "1",
      reasonArray: reason,
      isModalOpen: !this.state.isModalOpen,
    });

    onGetDecisionsRulesReasons(reason.Id);
    onGetDecisionsRulesCanceledReasons(reason.Id);
  };
  componentDidMount() {
    const {
      decisions,
      onGetDecisions,
      deleted,
      decisionCategories,
      onGetRoles,
      user_menu,
    } = this.props;
    this.updateShowAddButton(user_menu, this.props.location.pathname);
    this.updateShowDeleteButton(user_menu, this.props.location.pathname);
    this.updateShowEditButton(user_menu, this.props.location.pathname);
    this.updateShowSearchButton(user_menu, this.props.location.pathname);
    if (decisions && !decisions.length) {
      onGetDecisions();
      onGetRoles();
      this.setState({ decisions });
      this.setState({ deleted });
      this.setState({ decisionCategories });
    }
  }

  componentDidUpdate(prevProps, prevState, snapshot) {
    if (
      this.props.user_menu !== prevProps.user_menu ||
      this.props.location.pathname !== prevProps.location.pathname
    ) {
      this.updateShowAddButton(
        this.props.user_menu,
        this.props.location.pathname
      );
      this.updateShowDeleteButton(
        this.props.user_menu,
        this.props.location.pathname
      );
      this.updateShowEditButton(
        this.props.user_menu,
        this.props.location.pathname
      );
      this.updateShowSearchButton(
        this.props.user_menu,
        this.props.location.pathname
      );
    }
  }

  updateShowAddButton = (menu, pathname) => {
    const showAddButton = checkIsAddForPage(menu, pathname);
    this.setState({ showAddButton });
  };

  updateShowDeleteButton = (menu, pathname) => {
    const showDeleteButton = checkIsDeleteForPage(menu, pathname);
    this.setState({ showDeleteButton });
  };

  updateShowEditButton = (menu, pathname) => {
    const showEditButton = checkIsEditForPage(menu, pathname);
    this.setState({ showEditButton });
  };

  updateShowSearchButton = (menu, pathname) => {
    const showSearchButton = checkIsSearchForPage(menu, pathname);
    this.setState({ showSearchButton });
  };

  onPaginationPageChange = page => {
    if (
      this.node &&
      this.node.current &&
      this.node.current.props &&
      this.node.current.props.pagination &&
      this.node.current.props.pagination.options
    ) {
      this.node.current.props.pagination.options.onPageChange(page);
    }
  };
  toggleDeleteModal = () => {
    this.setState(prevState => ({
      deleteModal: !prevState.deleteModal,
    }));
  };

  onClickDelete = rowId => {
    this.setState({ selectedRowId: rowId, deleteModal: true });
  };

  handleAddRow = () => {
    const { onAddNewDecision, decisions } = this.props;

    const newRow = {
      arTitle: "-----",
      enTitle: "",
    };

    const emptyRowsExist = decisions.some(
      decision => decision.arTitle.trim() === "-----"
    );

    if (emptyRowsExist) {
      const errorMessage = this.props.t("Fill in the empty row");
      this.setState({ duplicateError: errorMessage });
    } else {
      this.setState({ duplicateError: null });
      onAddNewDecision(newRow);
    }
  };
  handleAddDecisionRow = num => {
    const {
      onAddNewDecisionsRulesCanceledReason,
      onAddNewDecisionsRulesReason,
      decisionRulesCanceledReasons,
      decisionRulesReasons,
    } = this.props;
    const { reasonArray } = this.state;
    const newRow = {
      decisionsRulesId: reasonArray.Id,
      arTitle: "-----",
      enTitle: "",
    };

    const emptyRowsExist =
      num === 1
        ? decisionRulesReasons.some(
            decision => decision.arTitle.trim() === "-----"
          )
        : num === 2
        ? decisionRulesCanceledReasons.some(
            decision => decision.arTitle.trim() === "-----"
          )
        : false;

    if (emptyRowsExist) {
      const errorMessage = this.props.t("Fill in the empty row");
      this.setState({ duplicateDecisionError: errorMessage });
    } else {
      this.setState({ duplicateDecisionError: null });
      if (num === 1) {
        onAddNewDecisionsRulesReason(newRow);
      } else if (num === 2) {
        onAddNewDecisionsRulesCanceledReason(newRow);
      }
    }
  };
  handleDeleteRow = () => {
    const { onDeleteDecision } = this.props;
    const { selectedRowId } = this.state;

    if (selectedRowId !== null) {
      onDeleteDecision(selectedRowId);

      this.setState({
        selectedRowId: null,
        deleteModal: false,
        showAlert: true,
      });
    }
  };
  onClickDeleteDecision = decision => {
    const {
      onDeleteDecisionsRulesReason,
      onDeleteDecisionsRulesCanceledReason,
    } = this.props;
    const { activeTab } = this.state;
    if (activeTab === "1") {
      onDeleteDecisionsRulesReason({ Id: decision.Id });
    } else if (activeTab === "2") {
      onDeleteDecisionsRulesCanceledReason({ Id: decision.Id });
    }
  };

  handleDecisionDataChange = (rowId, fieldName, fieldValue) => {
    const { onUpdateDecision, decisions } = this.props;

    const isDuplicate = decisions.some(
      decision =>
        decision.Id !== rowId &&
        ((decision.arTitle && decision.arTitle.trim() === fieldValue.trim()) ||
          (decision.enTitle && decision.enTitle.trim() === fieldValue.trim()))
    );

    if (isDuplicate) {
      const errorMessage = this.props.t("Value already exists");
      this.setState({ duplicateError: errorMessage });
      let onUpdate = { Id: rowId, [fieldName]: "-----" };
      onUpdateDecision(onUpdate);
    } else {
      this.setState({ duplicateError: null });
      let onUpdate = { Id: rowId, [fieldName]: fieldValue };
      onUpdateDecision(onUpdate);
    }
  };

  handleDecisionRulesDataChange = (rowId, fieldName, fieldValue, num) => {
    const {
      onUpdateDecisionsRulesCanceledReason,
      onUpdateDecisionsRulesReason,
      decisionRulesReasons,
      decisionRulesCanceledReasons,
    } = this.props;
    const isDuplicate =
      num === 1
        ? decisionRulesReasons.some(
            decision =>
              decision.Id !== rowId &&
              ((decision.arTitle &&
                decision.arTitle.trim() === fieldValue.trim()) ||
                (decision.enTitle &&
                  decision.enTitle.trim() === fieldValue.trim()))
          )
        : num === 2
        ? decisionRulesCanceledReasons.some(
            decision =>
              decision.Id !== rowId &&
              ((decision.arTitle &&
                decision.arTitle.trim() === fieldValue.trim()) ||
                (decision.enTitle &&
                  decision.enTitle.trim() === fieldValue.trim()))
          )
        : false;
    if (isDuplicate) {
      const errorMessage = this.props.t("Value already exists");
      this.setState({ duplicateDecisionError: errorMessage });
      let onUpdate = { Id: rowId, [fieldName]: "-----" };
      if (num === 1) {
        onUpdateDecisionsRulesReason(onUpdate);
      } else if (num === 2) {
        onUpdateDecisionsRulesCanceledReason(onUpdate);
      }
    } else {
      this.setState({ duplicateDecisionError: null });
      let onUpdate = { Id: rowId, [fieldName]: fieldValue };
      if (num === 1) {
        onUpdateDecisionsRulesReason(onUpdate);
      } else if (num === 2) {
        onUpdateDecisionsRulesCanceledReason(onUpdate);
      }
    }
  };
  handleAlertClose = () => {
    this.setState({ duplicateError: null });
  };
  handleAlertCloseDecision = () => {
    this.setState({ duplicateDecisionError: null });
  };
  handleSuccessClose = () => {
    const { onGetDecisionDeletedValue } = this.props;
    this.setState({ showAlert: null });
    onGetDecisionDeletedValue();
  };

  handleErrorClose = () => {
    const { onGetDecisionDeletedValue } = this.props;
    this.setState({ showAlert: null });
    onGetDecisionDeletedValue();
  };
  handleSelectChangeDetails = (rowId, fieldName, selectedValue) => {
    const { onUpdateDecision } = this.props;

    let onUpdate = { Id: rowId, [fieldName]: selectedValue };
    onUpdateDecision(onUpdate);
  };
  handleChangeCheckbox = (row, fieldName) => {
    const { onUpdateDecision } = this.props;
    const newStatus = row[fieldName] ? 0 : 1;
    let ob = {
      Id: row.Id,
      [fieldName]: newStatus,
    };

    onUpdateDecision(ob);
  };

  handleMulti = selectedMulti => {
    const { onAddNewDecisionsRulesRole } = this.props;
    const { reasonArray, activeTab, decisionObj } = this.state;
    const selectedMultiWithId = selectedMulti.map(item => ({
      ...item,
      value1: reasonArray.Id,
    }));

    let newRoleDecisions = {
      multiArray: selectedMultiWithId,
      Id: decisionObj.Id,
    };

    if (activeTab === "3") {
      newRoleDecisions["tablename"] = "settings_DecisionsRulesAcceptRoles";
      onAddNewDecisionsRulesRole(newRoleDecisions);
      newRoleDecisions = {};
    } else if (activeTab === "4") {
      newRoleDecisions["tablename"] = "settings_DecisionsRulesRefuseRoles";
      onAddNewDecisionsRulesRole(newRoleDecisions);
      newRoleDecisions = {};
    }
  };
  render() {
    const {
      decisions,
      t,
      deleted,
      decisionCategories,
      decisionRulesCanceledReasons,
      decisionRulesReasons,
      roles,
      decisionsRulesRoles,
    } = this.props;
    const {
      duplicateError,
      duplicateDecisionError,
      deleteModal,
      showAlert,
      decisionObj,
      reasonArray,
      showAddButton,
      showDeleteButton,
      showEditButton,
      showSearchButton,
    } = this.state;
    const alertMessage =
      deleted == 0
        ? this.props.t("Can't Delete")
        : this.props.t("Deleted Successfully");

    const { SearchBar } = Search;
    const defaultSorting = [
      {
        dataField: "Id",
        order: "desc",
      },
    ];
    const columns = [
      { dataField: "Id", text: t("ID"), hidden: true },
      {
        dataField: "arTitle",
        text: t("Decision(ar)"),
        sort: true,
        editable: showEditButton,
      },
      {
        dataField: "enTitle",
        text: "Decision",
        sort: true,
        editable: showEditButton,
      },
      {
        dataField: "isCalAvg",
        text: t("Calculate Average"),
        sort: true,
        editable: false,
        formatter: (cellContent, row, column) => (
          <Input
            type="checkbox"
            name="isCalAvg"
            className={`form-check-input input-mini warning}`}
            id="isCalAvg"
            defaultChecked={cellContent == 1}
            onChange={event => this.handleChangeCheckbox(row, "isCalAvg")}
            disabled={!showEditButton}
          />
        ),
      },
      {
        dataField: "decisionCategoryId",
        text: t("Category"),
        formatter: (cell, row) => (
          <Select
            key={`decisionCategoryId`}
            options={decisionCategories}
            onChange={newValue => {
              this.handleSelectChangeDetails(
                row.Id,
                "decisionCategoryId",
                newValue.value
              );
            }}
            value={decisionCategories.find(
              opt => opt.value == row.decisionCategoryId
            )}
            isDisabled={!showEditButton}
          />
        ),
        editable: false,
      },
      {
        dataField: "delete",
        text: "",
        isDummyField: true,
        editable: false,
        formatter: (cellContent, decision) => (
          <div className="d-flex align-items-center">
            <Tooltip title={this.props.t("Decision Settings")} placement="top">
              <Link className="d-flex align-items-center" to="#">
                <i
                  className="dripicons-gear font-size-16 "
                  id="deletetooltip"
                  onClick={() => this.onClickReason(decision)}
                ></i>
              </Link>
            </Tooltip>
            {showDeleteButton && (
              <Link className="text-danger" to="#">
                <i
                  className="mdi mdi-delete font-size-18"
                  id="deletetooltip"
                  onClick={() => this.onClickDelete(decision)}
                ></i>
              </Link>
            )}
          </div>
        ),
      },
    ];
    const pageOptions = {
      sizePerPage: 10,
      totalSize: decisions.length,
      custom: true,
    };
    const decisionsColumns = [
      { dataField: "Id", text: t("ID"), hidden: true },
      {
        dataField: "arTitle",
        text: t("Decision(ar)"),
        sort: true,
        editable: showEditButton,
      },
      {
        dataField: "enTitle",
        text: "Decision",
        sort: true,
        editable: showEditButton,
      },
      {
        dataField: "delete",
        text: "",
        isDummyField: true,
        editable: false,
        formatter: (cellContent, decision) => (
          <div>
            {showDeleteButton && (
              <Link className="text-danger" to="#">
                <i
                  className="mdi mdi-delete font-size-18"
                  id="deletetooltip"
                  onClick={() => this.onClickDeleteDecision(decision)}
                ></i>
              </Link>
            )}
          </div>
        ),
      },
    ];

    const rolesModified = roles.map(item => ({
      value: item.Id,
      label: item.arTitle,
    }));
    return (
      <React.Fragment>
        <Modal
          isOpen={this.state.isModalOpen}
          toggle={this.toggleModal}
          size="lg"
        >
          <ModalHeader toggle={this.toggleModal} tag="h4">
            {reasonArray && reasonArray.arTitle}
          </ModalHeader>
          <ModalBody>
            <Nav pills className="navtab-bg nav-justified">
              <NavItem>
                <NavLink
                  to="#"
                  id="vertical-home-link-test"
                  className={classnames({
                    active: this.state.activeTab === "1",
                  })}
                  onClick={() => {
                    this.toggleTab("1");
                  }}
                >
                  {this.props.t("Decision Reasons")}
                </NavLink>
              </NavItem>
              <NavItem>
                <NavLink
                  to="#"
                  id="vertical-home-link-test"
                  className={classnames({
                    active: this.state.activeTab === "2",
                  })}
                  onClick={() => {
                    this.toggleTab("2");
                  }}
                >
                  {this.props.t("Canceled Decision Reasons")}
                </NavLink>
              </NavItem>
              <NavItem>
                <NavLink
                  to="#"
                  id="vertical-home-link-test"
                  className={classnames({
                    active: this.state.activeTab === "3",
                  })}
                  onClick={() => {
                    this.toggleTab("3");
                  }}
                >
                  {this.props.t("Accepted Roles for Granting")}
                </NavLink>
              </NavItem>
              <NavItem>
                <NavLink
                  to="#"
                  id="vertical-home-link-test"
                  className={classnames({
                    active: this.state.activeTab === "4",
                  })}
                  onClick={() => {
                    this.toggleTab("4");
                  }}
                >
                  {this.props.t("Accepted Roles for Deletion")}
                </NavLink>
              </NavItem>
            </Nav>
            <TabContent className="p-4" activeTab={this.state.activeTab}>
              <TabPane tabId="1">
                {duplicateDecisionError && (
                  <Alert
                    color="danger"
                    className="d-flex justify-content-center align-items-center alert-dismissible fade show"
                    role="alert"
                  >
                    {duplicateDecisionError}
                    <button
                      type="button"
                      className="btn-close"
                      aria-label="Close"
                      onClick={this.handleAlertCloseDecision}
                    ></button>
                  </Alert>
                )}
                <BootstrapTable
                  keyField="Id"
                  data={decisionRulesReasons}
                  columns={decisionsColumns}
                  cellEdit={cellEditFactory({
                    mode: "click",
                    blurToSave: true,
                    afterSaveCell: (oldValue, newValue, row, column) => {
                      this.handleDecisionRulesDataChange(
                        row.Id,
                        column.dataField,
                        newValue,
                        1
                      );
                    },
                  })}
                  noDataIndication={t("No Decisions found")}
                  defaultSorted={defaultSorting}
                />
                <Row>
                  <Col sm="12">
                    <div className="text-sm-end">
                      <Tooltip title={this.props.t("Add")} placement="top">
                        <IconButton
                          color="primary"
                          onClick={() => this.handleAddDecisionRow(1)}
                        >
                          <i className="mdi mdi-plus-circle blue-noti-icon" />
                        </IconButton>
                      </Tooltip>
                    </div>
                  </Col>
                </Row>
              </TabPane>
              <TabPane tabId="2">
                {duplicateDecisionError && (
                  <Alert
                    color="danger"
                    className="d-flex justify-content-center align-items-center alert-dismissible fade show"
                    role="alert"
                  >
                    {duplicateDecisionError}
                    <button
                      type="button"
                      className="btn-close"
                      aria-label="Close"
                      onClick={this.handleAlertCloseDecision}
                    ></button>
                  </Alert>
                )}
                <BootstrapTable
                  keyField="Id"
                  data={decisionRulesCanceledReasons}
                  columns={decisionsColumns}
                  cellEdit={cellEditFactory({
                    mode: "click",
                    blurToSave: true,
                    afterSaveCell: (oldValue, newValue, row, column) => {
                      this.handleDecisionRulesDataChange(
                        row.Id,
                        column.dataField,
                        newValue,
                        2
                      );
                    },
                  })}
                  noDataIndication={t("No Decisions found")}
                  defaultSorted={defaultSorting}
                />
                <Row>
                  <Col sm="12">
                    <div className="text-sm-end">
                      <Tooltip title={this.props.t("Add")} placement="top">
                        <IconButton
                          color="primary"
                          onClick={() => this.handleAddDecisionRow(2)}
                        >
                          <i className="mdi mdi-plus-circle blue-noti-icon" />
                        </IconButton>
                      </Tooltip>
                    </div>
                  </Col>
                </Row>
              </TabPane>
              <TabPane tabId="3">
                <Select
                  options={rolesModified}
                  defaultValue={decisionObj ? decisionObj.AcceptRoles : []}
                  onChange={this.handleMulti}
                  isMulti
                  isDisabled={!showEditButton}
                />
              </TabPane>
              <TabPane tabId="4">
                <Select
                  options={rolesModified}
                  defaultValue={decisionObj ? decisionObj.RefuseRoles : []}
                  onChange={this.handleMulti}
                  isMulti
                  isDisabled={!showEditButton}
                />
              </TabPane>
            </TabContent>
          </ModalBody>
        </Modal>
        <DeleteModal
          show={deleteModal}
          onDeleteClick={this.handleDeleteRow}
          onCloseClick={() =>
            this.setState({ deleteModal: false, selectedRowId: null })
          }
        />
        <div className="page-content">
          <div className="container-fluid">
            <Breadcrumbs
              title={t("Decisions")}
              breadcrumbItem={t("Decisions List")}
            />
            <Row>
              <Col>
                <Card>
                  <CardBody>
                    <div>
                      {duplicateError && (
                        <Alert
                          color="danger"
                          className="d-flex justify-content-center align-items-center alert-dismissible fade show"
                          role="alert"
                        >
                          {duplicateError}
                          <button
                            type="button"
                            className="btn-close"
                            aria-label="Close"
                            onClick={this.handleAlertClose}
                          ></button>
                        </Alert>
                      )}
                      {deleted == 0 && showAlert && (
                        <Alert
                          color="danger"
                          className="d-flex justify-content-center align-items-center alert-dismissible fade show"
                          role="alert"
                        >
                          {alertMessage}
                          <button
                            type="button"
                            className="btn-close"
                            aria-label="Close"
                            onClick={this.handleErrorClose}
                          ></button>
                        </Alert>
                      )}
                      {deleted == 1 && showAlert && (
                        <Alert
                          color="success"
                          className="d-flex justify-content-center align-items-center alert-dismissible fade show"
                          role="alert"
                        >
                          {alertMessage}
                          <button
                            type="button"
                            className="btn-close"
                            aria-label="Close"
                            onClick={this.handleSuccessClose}
                          ></button>
                        </Alert>
                      )}
                    </div>
                    <div className="table-responsive">
                      <PaginationProvider
                        pagination={paginationFactory(pageOptions)}
                        keyField="Id"
                        columns={columns}
                        data={decisions}
                      >
                        {({ paginationProps, paginationTableProps }) => (
                          <ToolkitProvider
                            keyField="Id"
                            data={decisions}
                            columns={columns}
                            search
                          >
                            {toolkitprops => (
                              <React.Fragment>
                                <Row>
                                  <Col sm="4">
                                    <div className="search-box ms-2 mb-2 d-inline-block">
                                      {showSearchButton && (
                                        <div className="position-relative">
                                          <SearchBar
                                            {...toolkitprops.searchProps}
                                          />
                                        </div>
                                      )}
                                    </div>
                                  </Col>
                                  <Col sm="8">
                                    {showAddButton && (
                                      <div className="text-sm-end">
                                        <Tooltip
                                          title={this.props.t("Add")}
                                          placement="top"
                                        >
                                          <IconButton
                                            color="primary"
                                            onClick={this.handleAddRow}
                                          >
                                            <i className="mdi mdi-plus-circle blue-noti-icon" />
                                          </IconButton>
                                        </Tooltip>
                                      </div>
                                    )}
                                  </Col>
                                </Row>

                                <BootstrapTable
                                  keyField="Id"
                                  {...toolkitprops.baseProps}
                                  {...paginationTableProps}
                                  data={decisions}
                                  columns={columns}
                                  cellEdit={cellEditFactory({
                                    mode: "click",
                                    blurToSave: true,
                                    afterSaveCell: (
                                      oldValue,
                                      newValue,
                                      row,
                                      column
                                    ) => {
                                      this.handleDecisionDataChange(
                                        row.Id,
                                        column.dataField,
                                        newValue
                                      );
                                    },
                                  })}
                                  noDataIndication={t("No Decisions found")}
                                  defaultSorted={defaultSorting}
                                />
                                <Col className="pagination pagination-rounded justify-content-end mb-2">
                                  <PaginationListStandalone
                                    {...paginationProps}
                                  />
                                </Col>
                              </React.Fragment>
                            )}
                          </ToolkitProvider>
                        )}
                      </PaginationProvider>
                    </div>
                  </CardBody>
                </Card>
              </Col>
            </Row>
          </div>
        </div>
      </React.Fragment>
    );
  }
}

const mapStateToProps = ({ decisions, roles, menu_items }) => ({
  decisions: decisions.decisions,
  deleted: decisions.deleted,
  decisionCategories: decisions.decisionCategories,
  decisionRulesReasons: decisions.decisionRulesReasons,
  decisionRulesCanceledReasons: decisions.decisionRulesCanceledReasons,
  roles: roles.roles,
  decisionsRulesRoles: decisions.decisionsRulesRoles,
  user_menu: menu_items.user_menu || [],
});

const mapDispatchToProps = dispatch => ({
  onGetDecisions: () => dispatch(getDecisions()),
  onGetRoles: () => dispatch(getRoles()),
  onAddNewDecision: decision => dispatch(addNewDecision(decision)),
  onUpdateDecision: decision => dispatch(updateDecision(decision)),
  onDeleteDecision: decision => dispatch(deleteDecision(decision)),
  onGetDecisionDeletedValue: () => dispatch(getDecisionDeletedValue()),
  onGetDecisionsRulesReasons: decisionId =>
    dispatch(getDecisionsRulesReasons(decisionId)),
  onAddNewDecisionsRulesReason: decision =>
    dispatch(addNewDecisionsRulesReason(decision)),
  onUpdateDecisionsRulesReason: decision =>
    dispatch(updateDecisionsRulesReason(decision)),
  onDeleteDecisionsRulesReason: decision =>
    dispatch(deleteDecisionsRulesReason(decision)),
  onGetDecisionsRulesReasonDeletedValue: () =>
    dispatch(getDecisionsRulesReasonDeletedValue()),
  onGetDecisionsRulesCanceledReasons: decisionId =>
    dispatch(getDecisionsRulesCanceledReasons(decisionId)),
  onAddNewDecisionsRulesCanceledReason: decision =>
    dispatch(addNewDecisionsRulesCanceledReason(decision)),
  onUpdateDecisionsRulesCanceledReason: decision =>
    dispatch(updateDecisionsRulesCanceledReason(decision)),
  onDeleteDecisionsRulesCanceledReason: decision =>
    dispatch(deleteDecisionsRulesCanceledReason(decision)),
  onGetDecisionsRulesCanceledReasonDeletedValue: () =>
    dispatch(getDecisionsRulesCanceledReasonDeletedValue()),
  onGetDecisionsRulesRoles: decisionId =>
    dispatch(getDecisionsRulesRoles(decisionId)),
  onAddNewDecisionsRulesRole: decision =>
    dispatch(addNewDecisionsRulesRole(decision)),
  onUpdateDecisionsRulesRole: decision =>
    dispatch(updateDecisionsRulesRole(decision)),
  onDeleteDecisionsRulesRole: decision =>
    dispatch(deleteDecisionsRulesRole(decision)),
  onGetDecisionsRulesRoleDeletedValue: () =>
    dispatch(getDecisionsRulesRoleDeletedValue()),
});

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTranslation()(DecisionsList));
