...
class DistributingCoursesMethods extends Component {
  ...
  handleDistributingCoursesMethodEditForm = arg => {
    ...
    if (distributingCoursesMethod && !distributingCoursesMethod.length) {
      ...
      onGetDistributingCourses(distributingCoursesMethod);
    }
  ...
  handleSave = values => {
    ...
    const variables = {
      ...
      yearSemesterId: parseInt(currentSemester.cuYearSemesterId) || 0,
    };
  ...
  handleInputChange = event => {
    ...
    const courseObject = courses.find(
      course =>  `${course.value} ${course.code}` === event.target.value
    );

    if (courseObject) {
      this.setState({ selectedCoursId: courseObject.key });
    }
  };
  ...
  render() {
    ...
    const CourseColumns = [
...

      {
        ...
        formatter: (cell, row) => {
          const filteredCourses = courses.filter(
            course => course.key === row.courseId
          );
            ...
            <div className="col-9">
              <div>
                {filteredCourses.map(filteredCourse => (
                  <label key={filteredCourse.key} value={filteredCourse.value}>
      ...
      },
      {
        ...
        editable: false,
        hidden: !showDeleteButton,
        formatter: (cellContent, distributingCourse) => (
          <Link className="text-danger" to="#">
            <i
              className="mdi mdi-delete font-size-18"
              id="deletetooltip"
              onClick={() => this.onClickDeleteCourse(distributingCourse)}
            ></i>
          </Link>
        ),
      },
                                                    ...
                                                    <div className="d-flex align-items-center">
                                                      ...
                                                      <datalist id="datalistOptions1">
                                                        {courses.map(course => (
                                                          <option
                                                            key={course.key}
                                                            value={`${course.value} ${course.code}`}
                                                          />
                                                        ))}
                                                      </datalist>
                                                      {showButtons && (
...
const mapDispatchToProps = dispatch => ({
  ...
  onGetDistributingCourses: distributingCourseMethods =>
    dispatch(getDistributingCourses(distributingCourseMethods)),

  onAddNewDistributingCourse: distributingCourse =>
    dispatch(addNewDistributingCourse(distributingCourse)),

  onDeleteDistributingCourse: distributingCourse =>
    dispatch(deleteDistributingCourse(distributingCourse)),
