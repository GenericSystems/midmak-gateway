...
*/
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.regReqDocuments !== this.props.regReqDocuments) {
      console.log("regReqDocuments updated:", this.props.regReqDocuments);
    }
  }
    ...
    const columns = [
      ...
      },
      {
        ...
        formatter: (cellContent, row) => (
          <Select
            key={`${row.Id}_select`}
            options={this.props.documents.filter(
              option =>
                !this.props.regReqDocuments.some(
                  doc => doc.documentTypeId === option.value
                ) || option.value === row.documentTypeId
            )}
            onChange={newValue => {
                              ...
                              <Row>
                                <Col>
                                  <input
                                    type="checkbox"
                                    className="btn-check"
                                    id={`btncheck${certificate.value}`}
                                    autoComplete="off"
                                    checked={checkedId === certificate.value}
                                    onChange={() =>
                                      this.handleShowColumn(
                                        `checked${certificate.value}`,
                                        certificate.value
                                      )
                                    }
                                  />
