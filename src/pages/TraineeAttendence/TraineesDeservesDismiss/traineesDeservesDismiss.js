...
} from "store/traineesDeservesDismiss/actions";
...
class TraineesDeservesDismissList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      employees: [],
      ...
      successMessage: null,
      values: "",
      modalTraineeDeserveDismissValue: [],
  ...
  componentDidMount() {
    ...
    // if (traineeDeserveDismiss && !traineesDeservesDismiss.length) {
    //   onGetTraineesDeservesDismiss();
    // }
    onGetTraineesDeservesDismiss();
    this.setState({
  ...
  toggle1(tab) {
    ...
    //   this.props.traineesDeservesDismiss
    // );
    // const { onGetTraineesDeservesDismiss } = this.props;
  ...
  handleDeleteRow = () => {
    const { onDeleteTraineeDeserveDismiss } = this.props;
    const { selectedRowId } = this.state;

    if (selectedRowId !== null) {
      let onDelete = { Id: selectedRowId.Id };
      onDeleteTraineeDeserveDismiss(onDelete);
  ...
  handleSuccessClose = () => {
    const { onGetTraineeDeserveDismissDeletedValue } = this.props;
  ...
  handleTraineeDeserveDismissClick = arg => {
    console.log("arg", arg);

    this.setState({
      traineeDeserveDismiss: arg,
      isEdit: true,
    });
    this.toggleEdit();
  };
  ...
  render() {
    const traineeDeserveDismiss = this.state.traineeDeserveDismiss;
    ...
    const selectRow = {
      mode: "checkbox",
      clickToSelect: false,
      selected: traineesDeservesDismiss
    ...
    const columns = [
      ...
      },
      {
        ...
        formatter: (cellContent, traineeDeserveDismiss) => (
          <Tooltip title={this.props.t("Edit")} placement="top">
            <IconButton
              onClick={() =>
                this.handleTraineeDeserveDismissClick(traineeDeserveDismiss)
              }
            >
    ...
    const pageOptions = {
      sizePerPage: 10,
      totalSize: traineesDeservesDismiss.length,
      custom: true,
    };
    return (
      <React.Fragment>
        ...
        <div className="page-content">
          <div className="container-fluid">
            <Breadcrumbs
              breadcrumbItem={this.props.t("Trainees Deserves Dismiss")}
            />
            <Row>
              <Col>
                <Card>
                      ...
                      <Col md="6">
                        <Nav pills className="navtab-bg nav-justified">
                          <NavItem>
                            ...
                            >
                              {this.props.t("Dismissed or Must Dismissed")}
                            </NavLink>
                          </NavItem>
                          <NavItem>
                            <NavLink
                            ...
                            >
                              {this.props.t("Must Dismissed Only")}
                            </NavLink>
                          </NavItem>
                          <NavItem>
                            <NavLink
                                  ...
                                  <React.Fragment>
                                    ...
                                    <BootstrapTable
                                      ...
                                      noDataIndication={this.props.t(
                                        "No Trainees Deserves Dismiss found"
                                      )}
...
const mapStateToProps = ({
  ...
  requestStatus: traineesDeservesDismiss.requestStatus,
  ...
  traineesDeservesDismiss: traineesDeservesDismiss.traineesDeservesDismiss,
  genders: employees.genders,
  employeesNames: employees.employeesNames,
  employees: employees.employees,
  deleted: traineesDeservesDismiss.deleted,
  user_menu: menu_items.user_menu || [],
});

const mapDispatchToProps = dispatch => ({
  onGetTraineesDeservesDismiss: () => dispatch(getTraineesDeservesDismiss()),
  onAddNewTraineeDeserveDismiss: traineeWarning =>
    dispatch(addNewTraineeDeserveDismiss(traineeWarning)),
  onUpdateTraineeDeserveDismiss: traineeWarning =>
    dispatch(updateTraineeDeserveDismiss(traineeWarning)),
  onDeleteTraineeDeserveDismiss: traineeWarning =>
    dispatch(deleteTraineeDeserveDismiss(traineeWarning)),
  onGetTraineeDeserveDismissDeletedValue: () =>
    dispatch(getTraineeDeserveDismissDeletedValue()),
});

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(withTranslation()(TraineesDeservesDismissList)));
