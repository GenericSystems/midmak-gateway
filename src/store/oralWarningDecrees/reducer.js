...
const oralWarningDecrees = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case UPDATE_ORAL_WARNING_DECREE_SUCCESS:
      return {
        ...state,
        oralWarningDecrees: state.oralWarningDecrees.map(oralWarningDecree =>
          oralWarningDecree.Id.toString() === action.payload.Id.toString()
            ? { ...oralWarningDecree, ...action.payload }
            : oralWarningDecree
        ),
      };
