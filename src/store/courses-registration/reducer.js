...
const coursesRegistration = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case ADD_COURSES_REGISTRATION_SUCCESS:
      return {
        ...state,
        coursesRegistration: [...state.coursesRegistration, action.payload],
      };

    case ADD_COURSES_REGISTRATION_FAIL:
      return {
        ...state,
        error: action.payload,
      };
    ...
    case ADD_AVAILABLE_COURSE_SUCCESS:
      const parsedPayload = {
        ...
        sections: JSON.parse(action.payload.sections).map(section => ({
          ...
          type: "section",
          rowId: action.payload.Id,
          details: section.SectionDetails=== null ? [] : section.SectionDetails,
    ...
    case UPDATE_COURSES_REGISTRATION_SUCCESS:
      return {
        ...state,
        coursesRegistration: state.coursesRegistration.map(
          coursesRegistration =>
            coursesRegistration.Id.toString() === action.payload.Id.toString()
              ? { coursesRegistration, ...action.payload }
              : coursesRegistration
        ),
      };

    case UPDATE_COURSES_REGISTRATION_FAIL:
      return {
        ...state,
        error: action.payload,
      };

    case DELETE_COURSES_REGISTRATION_SUCCESS:
      return {
        ...state,
    ...
    case DELETE_COURSES_REGISTRATION_FAIL:
      return {
        ...state,
        error: action.payload,
      };
